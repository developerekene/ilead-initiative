...
const Dashboard: React.FC = () => {
  ...
  const sidebarGroups: SidebarGroup[] = [
    ...
    },
    {
      ...
      items: [
        ...
        {
          label: "Academic Consultation Intake",
    ...
    },
    {
      ...
      items: [
        ...
        {
          label: "Create a Testimonial Video/Post",
  ...
  const statsOverview = [
    ...
    {
      label: "iShare Transferred",
      value: "450k",
      icon: "🔄",
      change: "Verified",
    },
    {
      label: "iTrain Modules Completed",
      value: "14 / 18",
      icon: "🧠",
      change: "82% progress",
    },
    {
      label: "Community Giveback Position",
      value: "Top 5%",
      icon: "🏆",
      change: "84 hrs gifted",
    },
    {
      label: "Active iNeed Requests",
      value: "1",
      icon: "🚨",
      change: "1 active offer",
    },
    {
      label: "Assigned Mentors",
      value: "2 Leaders",
      icon: "🤝",
      change: "Weekly sync active",
    },
    {
      label: "Current Current CGPA Metric",
      value: "3.84",
      icon: "📈",
      change: "Last updated today",
    },
    {
      label: "Vault Records Streamed",
      value: "42 Clips",
      icon: "📹",
      change: "1.2 GB consumed",
    },
    {
      label: "Certification Benchmarks",
      value: "2 Unlocked",
      icon: "📜",
  ...
  const userDisplayName =
    user?.displayName ||
    `${user?.firstName || "iLead"} ${user?.lastName || "Member"}`.trim();
  const userEmail = user?.email || "member@ilead-ecosystem.org";

  return (
    <div className="w-full h-full min-h-[calc(100vh-5rem)] bg-slate-50 flex border-t border-purple-950/5">
      {/* 2. SIDEBAR NAVIGATION COMPONENT (COLLAPSIBLE SECTION ARCHITECTURE) */}
      <aside className="w-72 bg-white border-r border-purple-950/5 hidden md:flex flex-col shrink-0 sticky top-20 h-[calc(100vh-5rem)] overflow-y-auto p-4 space-y-3">
        ...
        <nav className="space-y-2 flex-1">
          {sidebarGroups.map((group) => {
            ...
            return (
              <div
                key={group.title}
                className="border border-purple-950/[0.02] rounded-xl overflow-hidden bg-slate-50/50"
              >
                <button
                  type="button"
                ...
                >
                  ...
                  <span
                    className={`text-[10px] text-purple-950/30 transform transition-transform duration-200 ${isOpen ? "rotate-180 text-orange-500" : ""}`}
                  >
                    ▼
                  </span>
                </button>
      ...
      <main className="flex-1 w-full overflow-x-hidden">
        {/* Render child route if path is deeper than /dashboard, otherwise show default view */}
        {location.pathname !== "/dashboard" ? (
          ...
          <div className="p-6 md:p-10 max-w-6xl mx-auto space-y-8">
            ...
            <section className="space-y-4">
              <div>
                <h3 className="text-sm font-black text-purple-950 uppercase tracking-wider pl-0.5">
                  Ecosystem Snapshot
                </h3>
                <p className="text-xs text-purple-950/40 font-medium mt-0.5">
                  Cross-referenced operational analytics parsed from your active
                  profile components.
                </p>
              </div>
            ...
            <div className="bg-gradient-to-r from-orange-50 to-orange-100/50 border border-orange-200/60 rounded-2xl p-5 flex items-start gap-3 text-xs leading-relaxed font-medium text-purple-950/80">
              ...
              <div>
                <strong>Automated Synchronization Log:</strong> Your academic
                tracking indices, video record vault bandwidth allocation, and
                community contribution hours are updated dynamically every 24
                workflow hours. For immediate manual workspace queries, navigate
                directly to your corresponding group parameter link in the
                directory matrix.
              </div>
            </div>
          </div>
