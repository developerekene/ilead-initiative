...
interface TimeSlot {
    topic: string;
    branch: "Study Habits" | "CGPA Strategy" | "University Entry" | "Skill Choices";
    date: string;
    startTime: string;
    endTime: string;
    maxCapacity: number;
...
const ITrainScheduler: React.FC = () => {
    ...
    const [uiFeedback, setUiFeedback] = useState<{ type: "success" | "error" | null; message: string }>({
        type: null,
        message: ""
    });
    ...
    const handleFormSubmit = (e: React.FormEvent) => {
        ...
        if (formData.startTime >= formData.endTime) {
            setUiFeedback({
                type: "error",
                message: "Chronology Error: The Session end time must happen after the start time."
            });
            return;
        }
        ...
        if (hasOverlap) {
            setUiFeedback({
                type: "error",
                message: "Scheduling Collision: You already have an open consultancy or teaching slot within this exact timeframe."
            });
            return;
        }
        ...
        console.log("ITRAIN-006 -> Synchronizing entry backend matrix array:", synchronizedPayload);

        setUiFeedback({
            type: "success",
            message: "Success! Your availability matrix has synced with the public student workspace catalog."
        });
            ...
            <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                ...
                <div className="lg:col-span-7 bg-purple-900/40 backdrop-blur-md border border-white/10 rounded-[2rem] p-6 md:p-8 shadow-2xl">
                    <div className="mb-6">
                        ...
                        <h2 className="text-2xl font-black text-white tracking-tight mt-3">Open a Teaching Block</h2>
                        <p className="text-sm text-purple-200/60 font-medium mt-1">
                            Map out your availability window. Your open blocks sync instantly into the interactive student index directory.
                        </p>
                    </div>
                    ...
                    <form onSubmit={handleFormSubmit} className="space-y-5">
                        {/* Topic Field Input */}
                        <div>
                            <label className="text-xs font-bold text-purple-200/80 block mb-1.5 pl-1">Session Topic or Focus Title</label>
                            <input
                                type="text"
                                name="topic"
                                required
                        ...
                        <div>
                            <label className="text-xs font-bold text-purple-200/80 block mb-1.5 pl-1">Consulting Branch Metadata</label>
                            <select
                                name="branch"
                            ...
                            >
                                <option value="University Entry">University Entry Guidance</option>
                                <option value="Study Habits">Study Habits & Focus Calibration</option>
                                <option value="CGPA Strategy">CGPA Target Management</option>
                                <option value="Skill Choices">Pre-University Skill Selection</option>
                            </select>
                        </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label className="text-xs font-bold text-purple-200/80 block mb-1.5 pl-1">Target Date</label>
                                <input
                                    type="date"
                                    name="date"
                                    required
                            ...
                            <div>
                                <label className="text-xs font-bold text-purple-200/80 block mb-1.5 pl-1">Max Student Seat Capacity</label>
                                <input
                                    type="number"
                                    name="maxCapacity"
                                    required
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label className="text-xs font-bold text-purple-200/80 block mb-1.5 pl-1">Start Time (24h Clock)</label>
                                <input
                                    type="time"
                                    name="startTime"
                                    required
                            ...
                            <div>
                                <label className="text-xs font-bold text-purple-200/80 block mb-1.5 pl-1">End Time (24h Clock)</label>
                                <input
                                    type="time"
                                    name="endTime"
                                    required
                        ...
                        {uiFeedback.type && (
                            <div className={`p-4 rounded-xl border text-xs font-bold tracking-wide transition-all duration-200 ${uiFeedback.type === "success"
                ...
                <div className="lg:col-span-5 space-y-6">
                    <div className="bg-purple-900/20 border border-white/5 rounded-[2rem] p-6">
                        <h3 className="text-sm font-black text-white uppercase tracking-wider mb-4">Your Dynamic Public Ledger</h3>
                        <p className="text-xs text-purple-200/50 font-medium mb-4 leading-relaxed">
                            These workshop items have passed integrity criteria checks and are actively stream-broadcasting onto the live consumer catalog interfaces:
                        </p>
