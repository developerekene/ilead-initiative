    ...
    <section className="w-full max-w-7xl mx-auto px-4 py-12 grid grid-cols-1 lg:grid-cols-12 gap-12 items-start bg-white">
      ...
      <div className="lg:col-span-4 bg-white border border-purple-950/10 rounded-[2rem] p-6 shadow-xl shadow-purple-950/[0.02]">
        <div className="mb-6">
          ...
          <h2 className="text-xl font-black text-purple-950 tracking-tight mt-3">
            Certificate Engine
          </h2>
          <p className="text-xs text-purple-950/50 font-medium mt-1">
            Input milestone parameters to generate high-resolution, on-demand
            credential layers dynamically.
          </p>
        </div>
        ...
        <form onSubmit={triggerGeneration} className="space-y-4">
          ...
          <div>
            <label className="text-xs font-bold text-purple-950/70 block mb-1">
              Recipient Full Name
            </label>
            <input
              type="text"
        ...
        {isGenerated ? (
            ...
            >
              ...
              <div className="text-center my-auto px-4 z-10">
                ...
                <p className="text-xs text-purple-950/70 font-medium max-w-lg mx-auto leading-relaxed mt-4">
                  {metaData.credentialType === "instructor" ? (
                    <>
                      has acted as a verified{" "}
                      <span className="text-purple-900 font-extrabold uppercase tracking-wide bg-purple-50 px-1.5 py-0.5 rounded border border-purple-100">
                        Certified Instructor/Consultant
                      </span>{" "}
                      inside Knowledge City, contributing over{" "}
                      <span className="font-bold text-purple-950 underline decoration-orange-500 decoration-2">
                        {metaData.totalHours} hours
                      </span>{" "}
                      of direct educational consultation, grade strategy
                      mentorship, and study habit support.
                    </>
                  ) : (
                    <>
                      has successfully completed the requirements for the{" "}
                      <span className="text-orange-600 font-extrabold uppercase tracking-wide bg-orange-50 px-1.5 py-0.5 rounded border border-orange-100">
                        Graduate Learner
                      </span>{" "}
                      tracking matrix within the core mentorship branch of
                    </>
                  )}
                </p>

                <h4 className="text-sm font-black text-purple-950 tracking-normal mt-3 max-w-xl mx-auto line-clamp-2">
                  {metaData.milestoneTrack ||
                    "General Academic Development Track"}
                </h4>
              </div>
              ...
              <div className="grid grid-cols-3 items-end text-center mt-2 z-10">
                ...
                <div className="flex justify-center items-center">
                  <div
                    className={`w-12 h-12 rounded-full border flex items-center justify-center bg-white shadow-sm transition-all duration-300 ${
                      metaData.credentialType === "instructor"
                        ? "border-purple-200"
                        : "border-orange-200"
                    }`}
          ...
          <div className="text-center py-20 bg-slate-50 border border-dashed border-purple-950/10 rounded-2xl w-full max-w-2xl">
            <p className="text-sm text-purple-950/40 font-medium">
              Configure properties to verify dynamic layout metrics.
            </p>
          </div>
        )}
      </div>
