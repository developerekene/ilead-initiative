...
const DIAGNOSTIC_QUESTIONS: Question[] = [
  {
    ...
    options: [
      {
        label:
          "I slice topics systematically using planned intervals (e.g., Pomodoro).",
        score: 3,
      },
      {
        label:
          "I study only when raw motivation strikes or close to deadlines.",
        score: 1,
      },
      {
        label:
          "I set broad goals but struggle to maintain consistent daily blocks.",
        score: 2,
      },
    ],
  },
  {
    ...
    options: [
      {
        label:
          "Rereading text segments, highlighting paragraphs, and hoping it sticks.",
        score: 1,
      },
      {
        label:
          "Testing myself blindly via flashcards, active blurting, and past prompts.",
        score: 3,
      },
      {
        label:
          "Summarizing chapters into shorter written notes without testing.",
        score: 2,
      },
    ],
  },
  {
    ...
    options: [
      {
        label:
          "Digital pings (instant phone notifications, messaging networks, background video tabs).",
        score: 1,
      },
      {
        label:
          "Physical workspaces (unpredictable noise, group disruptions, disorganized desks).",
        score: 2,
      },
      {
        label:
          "I maintain total structural insulation from external focus traps.",
        score: 3,
      },
    ],
  },
...
const StudyHabitsAssessment: React.FC = () => {
  ...
  const handleSubmitAssessment = async () => {
    ...
    try {
      ...
      console.log(
        "AC #1 - Payload saved safely inside profile schema:",
        assessmentPayload,
      );
      setIsSubmitted(true);
    } catch (error) {
  ...
  const getCategoryFeedback = (cat: "time" | "recall" | "environment") => {
    ...
    if (userScore === 3)
      return {
        ...
        tip: "Excellent foundational systems. Continue maintaining these continuous self-testing loops.",
      };
    if (userScore === 2)
      return {
        status: "Moderate Risk",
        color: "text-amber-600 bg-amber-50 border-amber-100",
        tip: "Inconsistent workflows detected. Try formalizing explicit, distraction-free scheduling metrics.",
      };
    return {
      status: "Critical Attention Needed",
      color: "text-rose-600 bg-rose-50 border-rose-100",
      tip: "Passive study routines increase friction. Shift entirely to proactive active recall or lock down digital traps.",
    };
  };
      ...
      <div className="bg-white border border-purple-950/5 rounded-[2.5rem] shadow-xl shadow-purple-950/5 overflow-hidden">
        ...
        <div className="bg-purple-950 px-8 py-10 text-white relative overflow-hidden">
          ...
          <p className="text-purple-200/70 text-xs sm:text-sm font-medium max-w-2xl mt-1 leading-relaxed">
            Complete this diagnostic track to identify critical workspace
            bottlenecks. Your generated profile summary is automatically
            appended to your incoming consultant session files.
          </p>
        </div>
        ...
        {!isSubmitted ? (
          <div className="p-8 md:p-12">
            ...
            <div className="flex justify-between items-center mb-8 text-xs font-bold text-purple-950/40 uppercase tracking-wider">
              <span>
                Step {currentStep + 1} of {DIAGNOSTIC_QUESTIONS.length}
              </span>
                ...
                {activeQuestion.options.map((option, index) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left p-4 rounded-xl border font-semibold text-sm transition-all duration-200 cursor-pointer flex items-center justify-between ${
                        isSelected
                    ...
                    >
                      <span className="leading-relaxed pr-4">
                        {option.label}
                      </span>
                      <div
            ...
            <div className="mt-10 pt-6 border-t border-purple-950/5 flex justify-between items-center">
              ...
              {currentStep === DIAGNOSTIC_QUESTIONS.length - 1 ? (
                <button
                  ...
                  disabled={
                    answers[activeQuestion.id] === undefined || isSyncing
                  }
                  className={`px-6 py-3 rounded-xl text-xs font-black uppercase tracking-widest text-white transition-all shadow-md cursor-pointer ${
                    answers[activeQuestion.id] === undefined || isSyncing
          ...
          <div className="p-8 md:p-12 animate-in fade-in duration-300">
            <div className="bg-emerald-50 border border-emerald-100 p-4 rounded-xl flex items-center gap-3 mb-8">
              ...
              <div>
                <h4 className="text-sm font-bold text-emerald-900">
                  Diagnostic Complete & Securely Appended
                </h4>
                <p className="text-xs text-emerald-700 font-medium">
                  Your metric blueprint has been synchronized onto your global
                  consultation request file automatically.
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
              {(["time", "recall", "environment"] as const).map((cat) => {
                ...
                return (
                  <div
                    key={cat}
                    className="border border-purple-950/5 rounded-2xl p-5 bg-slate-50/50 flex flex-col justify-between"
                  >
                    <div>
                      ...
                      <span
                        className={`inline-block px-2.5 py-0.5 rounded-md text-[11px] font-bold border ${analysis.color}`}
                      >
                        {analysis.status}
                      </span>
                    </div>
            ...
            <div className="pt-6 border-t border-purple-950/5 flex flex-col sm:flex-row justify-between items-center gap-4">
              <span className="text-xs font-medium text-purple-950/40 max-w-sm text-center sm:text-left">
                Ready to fix these boundaries? Proceed to lock down your target
                calendar block with a verified consultant.
              </span>
              <button
                type="button"
                onClick={() => (window.location.href = "/itrain/consultants")}
