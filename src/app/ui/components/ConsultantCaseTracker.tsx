...
const ConsultantCaseTracker: React.FC = () => {
  ...
  const moveStatus = (
    caseId: string,
    nextStatus: "Pending Review" | "Active Mentorship" | "Completed Tracks",
  ) => {
    setCases((prev) =>
      prev.map((item) =>
    ...
    const label =
      nextStatus === "Active Mentorship"
        ? "ACCEPTED"
        : nextStatus === "Completed Tracks"
          ? "COMPLETED"
          : "GENERAL";
    dispatch(
      addNotification({
  ...
  const triggerCgpaToast = (
    student: string,
    current: string,
    target: string,
  ) => {
    alert(
      `Opening CGPA Projection Engine for ${student}\nCurrent Matrix: ${current} → Target Runway: ${target}`,
    );
  };

  const triggerDiagnosticToast = (student: string, log: string) => {
    alert(
      `Opening Diagnostic Activity Logs for ${student}\nLatest Trace: "${log}"`,
    );
  };

  // Columns config object for rendering clean workflow blocks
  const columnDefinitions: {
    id: "Pending Review" | "Active Mentorship" | "Completed Tracks";
    label: string;
    badgeColor: string;
  }[] = [
    {
    ...
    <div className="w-full bg-white rounded-[2rem] border border-purple-950/10 p-6 md:p-8 shadow-xl shadow-purple-950/[0.01]">
      ...
      <div className="border-b border-purple-950/5 pb-5 mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <h3 className="text-xl font-black text-purple-950 tracking-tight mt-2">
            Exclusive Consultant Case Tracking Hub
          </h3>
          <p className="text-xs text-purple-950/50 font-medium mt-0.5">
            Here we track student diagnostics, update mentoring statuses, and
            inspect intake forms in real time.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {columnDefinitions.map((column) => {
          ...
          return (
            <div
              key={column.id}
              className="bg-slate-50/50 border border-purple-950/[0.02] rounded-2xl p-4 flex flex-col min-h-[450px]"
            >
              {/* Column Header */}
              <div className="flex items-center justify-between mb-4 pb-2 border-b border-purple-950/5">
                <span
                  className={`text-[11px] font-black uppercase tracking-wider px-2.5 py-0.5 rounded-md border ${column.badgeColor}`}
                >
                  {column.label}
                </span>
              ...
              <div className="space-y-3.5 flex-1 overflow-y-auto">
                {columnCases.length === 0 ? (
                  <div className="h-32 border border-dashed border-purple-950/10 rounded-xl flex flex-col items-center justify-center text-center p-4">
                    <span className="text-xs font-bold text-purple-950/30">
                  ...
                  columnCases.map((item) => (
                    ...
                    >
                      ...
                      <div className="bg-slate-50 border border-purple-950/[0.02] rounded-lg p-2.5 mb-3 text-[11px] font-medium text-purple-950/70 leading-relaxed">
                        <span className="font-bold text-purple-950/40 uppercase text-[9px] tracking-wider block mb-0.5">
                          Last Log Context
                        </span>
                      ...
                      <div className="grid grid-cols-2 gap-2 mb-4 pt-1">
                        ...
                        >
                          📊 CGPA Runway{" "}
                          <span className="font-black text-purple-950 bg-slate-100 px-1 rounded">
                            {item.currentCgpa}
                          </span>
                        </button>
                        <button
                      ...
                      <div className="border-t border-purple-950/5 pt-3 flex items-center justify-end gap-1.5">
                        <span className="text-[9px] font-bold text-purple-950/30 uppercase mr-auto select-none">
                          Route:
                        </span>
                        ...
                        {item.status !== "Pending Review" && (
                          <button
                            type="button"
                            onClick={() =>
                              moveStatus(item.id, "Pending Review")
                            }
                        ...
                        {item.status === "Pending Review" && (
                          <button
                            type="button"
                            onClick={() =>
                              moveStatus(item.id, "Active Mentorship")
                            }
                        ...
                        {item.status === "Active Mentorship" && (
                          <button
                            type="button"
                            onClick={() =>
                              moveStatus(item.id, "Completed Tracks")
                            }
