    ...
    useEffect(() => {
        function handleClickOutside(event: MouseEvent) {
            ...
            if (mobileMenuRef.current && !mobileMenuRef.current.contains(target)) {
                const clickedToggle = (target as HTMLElement).closest('.mobile-toggle-btn');
                if (!clickedToggle) {
    ...
    return (
        <header className="fixed top-0 left-0 w-full z-50 bg-white/80 backdrop-blur-md border-b border-orange-100 shadow-sm shadow-purple-500/5">
            <nav className="flex justify-between items-center px-6 py-4 md:px-12 max-w-7xl mx-auto">
                ...
                >
                    iLEAD <span className="text-orange-500 text-sm font-bold tracking-wider uppercase bg-orange-50 px-2 py-0.5 rounded-md border border-orange-100">Initiative</span>
                </Link>

                {/* Desktop Navigation Links */}
                <div className="hidden md:flex items-center gap-10">
                    <Link to="/features" className="text-sm font-semibold text-purple-950/60 hover:text-purple-900 transition-colors relative after:absolute after:bottom-[-4px] after:left-0 after:w-0 after:h-[2px] after:bg-orange-500 hover:after:w-full after:transition-all">
                        iShare
                    </Link>
                    <Link to="/features" className="text-sm font-semibold text-purple-950/60 hover:text-purple-900 transition-colors relative after:absolute after:bottom-[-4px] after:left-0 after:w-0 after:h-[2px] after:bg-orange-500 hover:after:w-full after:transition-all">
                        iTrain
                ...
                <div className="flex items-center gap-4 md:gap-6">
                    {/* FIXED: Path redirected to /join-our-commnity */}
                    <button
                        onClick={() => { closeAllMenus(); navigate('/join-our-commnity'); }}
                    ...
                    >
                        <div className="w-5 h-4 flex flex-col justify-between relative">
                            <span className={`w-full h-0.5 bg-purple-950 rounded-sm transition-all duration-300 origin-left ${isMobileMenuOpen ? 'rotate-45 translate-x-0.5 -translate-y-0.5' : ''}`} />
                            <span className={`w-full h-0.5 bg-purple-950 rounded-sm transition-opacity duration-200 ${isMobileMenuOpen ? 'opacity-0' : ''}`} />
                            <span className={`w-full h-0.5 bg-purple-950 rounded-sm transition-all duration-300 origin-left ${isMobileMenuOpen ? '-rotate-45 translate-x-0.5 translate-y-0.5' : ''}`} />
                        </div>
                    </button>
                </div>
            ...
            {/* Mobile/Tablet Drawer Layout Layer */}
            {isMobileMenuOpen && (
                <div
                    ref={mobileMenuRef}
                ...
                >
                    ...
                    {/* FIXED: Mobile CTA block mapped dynamically to /join-our-commnity */}
                    <button
                        onClick={() => { closeAllMenus(); navigate('/join-our-commnity'); }}
