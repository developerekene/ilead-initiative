...
const VideoTestimonialUpload: React.FC = () => {
    ...
    const handleUploadSubmit = (e: React.FormEvent) => {
        ...
        if (!selectedFile) {
            setFeedback({ type: "error", message: "Please drop or select a video to upload first." });
            return;
        }

        // Acceptance Criteria Check: Consent verification lock
        if (!giveConsent) {
            setFeedback({ type: "error", message: "Consent Required: You must accept the community showcase check-box to continue." });
            return;
        }
        ...
        const interval = setInterval(() => {
            setUploadProgress((prev) => {
                if (prev >= 100) {
                    ...
                    setFeedback({
                        type: "success",
                        message: "Thank you! Your video testimony has processed successfully and is synced with the community showcase network."
                    });
        ...
        <section className="w-full max-w-2xl mx-auto px-4 py-12">
            <div className="bg-white border border-purple-950/5 rounded-[2rem] p-6 md:p-8 shadow-xl shadow-purple-950/[0.02]">
                ...
                <div className="mb-6">
                    ...
                    <h2 className="text-2xl font-black text-purple-950 tracking-tight mt-3">Share Your Impact Story</h2>
                    <p className="text-sm text-purple-950/50 font-medium mt-1">
                        Encourage fellow students across the workspace. Record how adjusting your study habits or grades built your academic momentum.
                    </p>
                </div>
                ...
                <form onSubmit={handleUploadSubmit} className="space-y-6">
                    ...
                    <div
                        ...
                        className={`w-full border-2 border-dashed rounded-2xl p-8 text-center transition-all cursor-pointer flex flex-col items-center justify-center min-h-[220px] ${isDragging
                        ...
                        {!selectedFile ? (
                            <>
                                ...
                                <p className="text-sm font-bold text-purple-950">
                                    Drag and drop your testimony video, or <span className="text-orange-500 underline">browse</span>
                                </p>
                    ...
                    {isUploading && (
                        <div className="bg-slate-50 border border-purple-950/[0.02] rounded-xl p-4 space-y-2 animate-pulse">
                            <div className="flex justify-between items-center text-xs font-bold text-purple-950">
                                ...
                                <span className="font-black text-orange-500">{uploadProgress}%</span>
                            </div>
                            ...
                            <p className="text-[10px] text-purple-950/40 font-medium text-center pt-1">
                                Reassuring note: Do not exit or clear this viewport until the verification layer clears.
                            </p>
                        </div>
                    )}
                    ...
                    <div className="flex items-start gap-3 bg-slate-50 p-4 rounded-xl border border-purple-950/[0.02]">
                        ...
                        />
                        <label htmlFor="consentCheckbox" className="text-xs font-semibold text-purple-950/70 leading-relaxed cursor-pointer select-none">
                            I verify and grant absolute authorization to add this video file onto the public community showcase directory, enabling other platform learners to use my insights.
                        </label>
                    </div>

                    {/* Operational Responses Feedback Box */}
                    {feedback.type && !isUploading && (
                        <div className={`p-4 rounded-xl border text-xs font-bold tracking-wide transition-all duration-200 ${feedback.type === "success"
