...
interface LiveNotification {
    id: string;
    caseId: string;
    type: "ACCEPTED" | "RESCHEDULED" | "COMPLETED";
    title: string;
    message: string;
    timestamp: string;
    isUnread: boolean;
...
const NotificationCenter: React.FC = () => {
    ...
    // Mock incoming transactional data array stream tracking the active user notifications
    const [notifications, setNotifications] = useState<LiveNotification[]>([
        {
            ...
            message: "Your academic strategy review for 'Pre-University Skill Selection' has been locked in.",
            senderName: "Dr. Arinze",
        ...
        },
        {
            ...
            message: "The group sync for 'CGPA Target Management Masterclass' was moved to 16:00 UTC.",
        ...
        },
        {
            ...
            caseId: "case-304",
            type: "COMPLETED",
            title: "Track Marked Complete",
            message: "Your 'Study Habits & Focus Calibration' roadmap has been successfully finalized. Download your certificate.",
            senderName: "Ecosystem Audit",
            timestamp: "Yesterday",
            isUnread: false
        }
    ]);

    // SATISFIES CRITERIA: Computes count dynamically to drive the active notification bubble indicator
    ...
    const handleNotificationClick = (alert: LiveNotification) => {
        // Mutate local matrix array state to clear target unread flag configuration
        setNotifications(prev =>
            prev.map(n => (n.id === alert.id ? { ...n, isUnread: false } : n))
        );
    ...
    const getTypeStyles = (type: LiveNotification["type"]) => {
        switch (type) {
            case "ACCEPTED":
                return { bg: "bg-emerald-50", border: "border-emerald-100", text: "text-emerald-700", icon: "✓" };
            case "RESCHEDULED":
                return { bg: "bg-amber-50", border: "border-amber-100", text: "text-amber-700", icon: "⏰" };
            case "COMPLETED":
                return { bg: "bg-purple-50", border: "border-purple-100", text: "text-purple-700", icon: "🎓" };
        }
    };
    ...
    return (
        <div className="relative inline-block bg-white" ref={dropdownRef}>
            ...
            >
                ...
                <svg className="w-5 h-5 text-purple-950/70 group-hover:text-purple-950 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {isOpen && (
                <div className="absolute right-0 mt-3 w-80 sm:w-96 bg-white border border-purple-950/10 rounded-2xl shadow-xl shadow-purple-950/5 z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
                    ...
                    <div className="px-5 py-4 border-b border-purple-950/5 flex items-center justify-between bg-white">
                        <div>
                            <h3 className="text-sm font-black text-purple-950 tracking-tight">Ecosystem Ledger Events</h3>
                            ...
                            notifications.map((alert) => {
                                ...
                                return (
                                    <div
                                        ...
                                        className={`p-4 transition-all flex gap-3 items-start cursor-pointer hover:bg-slate-50 relative ${alert.isUnread ? "bg-orange-50/20" : "bg-white"
                                            }`}
                                    >
                                        ...
                                        <div className={`w-8 h-8 rounded-xl flex items-center justify-center font-bold text-xs shrink-0 border ${meta.bg} ${meta.border} ${meta.text}`}>
                                            {meta.icon}
                                        </div>
                                        ...
                                        <div className="flex-1 space-y-0.5">
                                            <div className="flex justify-between items-start gap-2">
                                                <h4 className={`text-xs font-black tracking-tight leading-snug ${alert.isUnread ? "text-purple-950" : "text-purple-950/80"
                                                    }`}>
                                                    {alert.title}
                                                </h4>
                                            ...
                                            <div className="pt-1 flex items-center gap-1.5 text-[10px] text-purple-950/40 font-bold uppercase tracking-wide">
                                                ...
                                                <span>•</span>
                                                <span className="text-purple-950/30">Ref ID: {alert.caseId}</span>
                                            </div>
                                        </div>
                                    </div>
                                );
