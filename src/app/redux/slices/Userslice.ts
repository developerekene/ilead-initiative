...
// ─────────────────────────────────────────────────────────────────────────────
// Pure Redux slice. No Firebase imports — all auth operations are delegated to
// authService.ts. This file only manages state shape and async thunks.
// ─────────────────────────────────────────────────────────────────────────────
...
} from "../../Authservice"; // adjust path to match your project
import type { SerializedUser } from "../../Authservice";
...
export const registerUser = createAsyncThunk(
  ...
  async (
    ...
    try {
      return await registerWithEmail(email, firstName, lastName);
    } catch (error: any) {
      return rejectWithValue(error.message as string);
    }
  },
...
const userSlice = createSlice({
  ...
  reducers: {
    /**
     * Called by AuthListener / onAuthStateChanged subscription.
     * Payload is pre-serialized — Immer never receives a Firebase class instance.
     */
    setUser(
      state,
      action: {
     ...
     * flips the flag instantly so Dashboard guard passes without
  ...
  extraReducers: (builder) => {
    ...
    // Fulfilled: auth thunks that return { user, profileComplete }
    builder
